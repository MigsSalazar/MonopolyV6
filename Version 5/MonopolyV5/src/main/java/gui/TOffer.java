...
public class TOffer extends JDialog implements ListSelectionListener{
	...
	public DefaultListModel<String> findAssets(){
		...
		for(String key : props.keySet()){
			if(props.get(key) instanceof Colored){
				if(((Colored)props.get(key)).getGrade()==0){
	...
	/**
	 * Gathers all the selected assets offered and returns them as an ArrayList of Asset objects
	 * @return	ArrayList of Assets containing all offered assets
	 */
	public ArrayList<Asset> offeredList(){
		ArrayList<Asset> offers = new ArrayList<Asset>();
		String a="";
		String t="";
		int w = 0;
		offers.add(new Asset(""+offeredCash, "cash", offeredCash));
		for(int i=0; i<oList.getModel().getSize(); i++){
			...
			if(a.contains("cash")){
				...
				for(String p : props.keySet()){
					if(p.equals(a)){
						w = props.get(p).getWorth();
					}
				}
			}
			offers.add( new Asset(a,t,w) );
		}
		return offers;
	}
